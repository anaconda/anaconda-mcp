...
sub-servers (conda) and remote proxied sub-servers (search), replacing the
previous ``ToolManager.call_tool`` monkeypatch (``patch_tool_call_hooks``).
"""
...
class PlatformMiddleware(Middleware):
    ...
    async def on_call_tool(self, context: MiddlewareContext, call_next):  # type: ignore[override]
        # --- auth enforcement (mirrors make_auth_enforcement_hook) ---
        ...
        # --- TOS enforcement (mirrors make_terms_enforcement_hook) ---
        verify_terms_accepted()
        # --- tool telemetry (mirrors make_tracked_call_tool) ---
...
def build_composed_server() -> FastMCP:
    ...
    search_client = Client(StreamableHttpTransport(search_url, auth=token), timeout=SEARCH_READ_TIMEOUT_SECONDS)
