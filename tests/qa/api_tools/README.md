...
The test session starts and stops the server automatically.
Requires `conda` in PATH and `anaconda-mcp` installed in the target env.
...
| `--server-conda-env` | `anaconda-mcp-rc-py313` | Conda env with `anaconda-mcp` (used with `--start-server`). |
...
# Auto-start with a specific server env and report metadata
python -m pytest tests/qa/api_tools/ -v \
  --start-server \
