...
anaconda-mcp claude install --config "$CONFIG_PATH"
cat "$CONFIG_PATH"
...
anaconda-mcp claude install --config "$CONFIG_PATH" --force
ls -la "$TEMP_DIR"
...
anaconda-mcp claude install --config "$CONFIG_PATH" --no-backup
